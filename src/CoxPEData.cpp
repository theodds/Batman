...
void CoxPEData::UpdateBase() {
  ...
  vec alpha = ones<vec>(K) * shape_haz;
  vec beta = ones<vec>(K) * rate_haz;

  // Calculate new shapes
  for(int i = 0; i < N; i++) {
    if(delta(i) == 1) {
      int bin = obs_to_bin(i);
      alpha(bin) = alpha(bin) + 1;
    }
  }
  ...
  for(int i = 0; i < N; i++) {
    ...
    for(int k = 0; k < K; k++) {
      if(bin == k) {
        beta(k) = beta(k) + (Y(i) - time_grid(k)) * r(i);
      }
      else if(bin > k) {
        beta(k) = beta(k) + (time_grid(k + 1) - time_grid(k)) * r(i);
      }
    }
  }
  ...
  // Sample things
  for(int k = 0; k < K; k++) {
    base_haz(k) = R::rgamma(alpha(k), 1.0  / beta(k));
  }

  cum_base_haz(0) = base_haz(0) * (time_grid(1) - time_grid(0));
  for(int k = 1; k < K; k++) {
    cum_base_haz(k) = cum_base_haz(k - 1) + base_haz(k) * (time_grid(k + 1) - time_grid(k));
  }
  ...
  for(int i = 0; i < N; i++) {
    ...
    for(int k = 0; k < K; k++) {
      if(bin == k) {
        cum_base_haz_Y(i) = cum_base_haz_Y(i) + base_haz(k) * (Y(i) - time_grid(k));
      }
      if(bin > k) {
        cum_base_haz_Y(i) = cum_base_haz_Y(i) + base_haz(k) * (time_grid(k+1) - time_grid(k));
      }
    }
  }

...
# 
# # Convert probabilities to ordinal outcomes
# # For simplicity, divide the probability range [0, 1] into n_categories
# # Generate thresholds and assign outcomes based on these thresholds
# thresholds <- seq(0, 1, length.out = n_categories + 1)[-c(1, n_categories + 2)]
# category <- findInterval(probabilities, vec = thresholds)
# 
# # Combine data into a data frame
# data <- data.frame(X)
# colnames(data) <- paste("feature", 1:n_features, sep = "_")
# data$outcome <- as.factor(category)
# 
